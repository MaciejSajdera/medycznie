<?php
/**
 * evenus functions and definitions
 *
...
if ( ! function_exists( 'evenus_setup' ) ) :
	/**
	 ...
	 */
	function evenus_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on evenus, use a find and replace
		 * to change 'evenus' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'evenus', get_template_directory() . '/assets/languages' );
...
add_action( 'after_setup_theme', 'evenus_setup' );
 ...
 */
function evenus_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'evenus_content_width', 640 );
}
add_action( 'after_setup_theme', 'evenus_content_width', 0 );
 ...
 */
function evenus_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'evenus' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'evenus' ),
...
add_action( 'widgets_init', 'evenus_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function evenus_scripts() {
	wp_enqueue_style( 'evenus-style', get_template_directory_uri() . '/dist/css/style.css' );

	wp_enqueue_script( 'evenus-app', get_template_directory_uri() . '/dist/js/main.js', array(), '', true );

	if (is_front_page()) {
		wp_enqueue_script( 'evenus-carousel', get_template_directory_uri() . '/dist/js/carousel.js', array(), '', true );
	}
...
add_action( 'wp_enqueue_scripts', 'evenus_scripts' );
...
function woocommerce_header_add_to_cart_fragment( $fragments ) {
	...
	<a class="cart-customlocation" href="<?php echo esc_url(wc_get_cart_url()); ?>" title="<?php _e('Pokaż koszyk', 'woothemes'); ?>">
		...
		<span class="cart-icon-wrapper">
			...
			<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" class="shop-icon"
